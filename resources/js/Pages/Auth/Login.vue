    ...
    <container :fit="true">
        ...
        <div>
            <form @submit.prevent="sendForm" class="flex flex-col items-center gap-y-4 mt-10">
                ...
                <div class="text-sm">
                    You do not have account yet? Click <Link :href="route('register')" class="text-purple-500 hover:underline font-semibold">here!</Link>
                </div>
            </form>
        </div>
    </container>
...
<script setup>
    ...
    const sendForm = () => {
        form.post(route('login_authenticate'));
    }
